...
module.exports = function (server, restify) {
		...
		if (CONFIG.dbmysql.enable) {
			...
			if (Array.isArray(CONFIG.dbmysql)) {
				...
				_MYSQL["MYSQL0"].getConnection(function (err, connection) {
					...
					console.log("MYSQL Initialized with _db2 - MYSQL0");
				});
			}
		}
	}
	...
	//Standard MySQL
	db2_query = function (dbkey, sql, params, callback) {
	...
	db2_selectQ = function (dbkey, table, columns, where, whereParams, callback, additionalQueryParams) {
		if (_MYSQL[dbkey] == null) {
			console.log("\x1b[31m%s\x1b[0m", `DATABASE Not Connected for ${dbkey}`);
			return callback(false);
		}
	...
	db2_insertQ1 = function (dbkey, table, data, connection, callback) {
		if (connection) {
			cols = [];
			quest = [];
			vals = [];
	...
	db2_insert_batchQ = function (dbkey, table, data, callback) {
		if (_MYSQL[dbkey] == null) {
			console.log("\x1b[31m%s\x1b[0m", `DATABASE Not Connected for ${dbkey}`);
			return callback(false);
		}
	...
	db2_deleteQ = function (dbkey, table, where, callback) {
		if (_MYSQL[dbkey] == null) {
			console.log("\x1b[31m%s\x1b[0m", `DATABASE Not Connected for ${dbkey}`);
			return callback(false);
		}
	...
	db2_updateQ = function (dbkey, table, data, where, connection, callback) {
		if (connection) {
			var fData = [];
			var vals = [];
	...
	db2_startTransaction = function (dbkey, callback) {
		console.log("db2_startTransaction - ", dbkey)
		if (_MYSQL[dbkey] == null) {
			console.log("\x1b[31m%s\x1b[0m", `DATABASE Not Connected for ${dbkey}`);
			return callback(false);
		}
		...
		_MYSQL[dbkey].getConnection(function (err, connection) {
			if (err || connection == null) {
				console.log("\x1b[31m%s\x1b[0m", `Error getting connection for ${dbkey}`);
				return callback(false);
			}

			connection.beginTransaction(function (err) {
				if (err) {
					console.log("\x1b[31m%s\x1b[0m", `Error starting transaction for ${dbkey}`);
					connection.release();
					return callback(false);
				}
				console.log(`Transaction started for ${dbkey}`);
				callback(null, connection);
			});
		});
	};

	db2_commitTransaction = function (connection, callback) {
		connection.commit(function (err) {
			if (err) {
				console.log("\x1b[31m%s\x1b[0m", `Error committing transaction`);
	...
	db2_rollbackTransaction = function (connection, callback) {
		connection.rollback(function () {
			console.log("\x1b[31m%s\x1b[0m", "Transaction rolled back");
			connection.release();
			callback(false);
		});
	};
